...
function criarPeriodos(materia, reset = false) {
  ...
  const anteriores = reset
    ? []
    : (materia.trimestres || []);

  materia.trimestres = Array.from(
    { length: config.quantidade },
    (_, index) => ({
      nome: `${index + 1}º ${config.nome}`,
      valores: [
        anteriores[index]?.valores?.[0] ?? ""
      ]
    })
  );
}
...
function normalizarDados() {
  ...
  dados.periodo =
    ["trimestre", "bimestre", "semestre"].includes(dados.periodo)
      ? dados.periodo
      : "trimestre";

  dados.mediaNecessaria =
    numero(dados.mediaNecessaria) ?? 60;

  dados.conceitos =
    Array.isArray(dados.conceitos)
      ? dados.conceitos
      : [];
  ...
  dados.conceitos = dados.conceitos
    .map(c => ({
      id: c.id || idNovo(),
  ...
  dados.materias = dados.materias
    .map(m => {
      const materia = {
        id: m.id || idNovo(),
        nome: String(m.nome || "").trim(),
        trimestres: Array.isArray(m.trimestres)
          ? m.trimestres
          : []
      };
      ...
      materia.trimestres = materia.trimestres.map((p, i) => ({
        nome:
          p.nome ||
          `${i + 1}º ${periodoConfig().nome}`,
        valores: [
          p?.valores?.[0] ?? ""
        ]
      }));

      if (!materia.trimestres.length) {
...
async function carregarBanco() {
  ...
  if (subjectIds.length) {
    const gradesResult =
      await supabase
        .from("grades")
        .select(
          "id, subject_id, period_index, value"
        )
        .in(
          "subject_id",
          subjectIds
  ...
  dados = {
    modo:
      settingsResult.data?.mode === "conceito"
        ? "conceito"
        : "nota",
    ...
    mediaNecessaria:
      numero(
        settingsResult.data?.required_points
      ) ?? 60,
...
async function migrarDadosLocais() {
  ...
  const materiasAntigas =
    Array.isArray(antigos.materias)
      ? antigos.materias
      : [];

  const conceitosAntigos =
    Array.isArray(antigos.conceitos)
      ? antigos.conceitos
      : [];

  const temDados =
    materiasAntigas.length ||
    conceitosAntigos.length;
  ...
  const config = {
    ...
    periodo:
      [
        ...
        "bimestre",
        "semestre"
      ].includes(antigos.periodo)
        ? antigos.periodo
        : "trimestre",

    mediaNecessaria:
      numero(antigos.mediaNecessaria) ?? 60
  };
    ...
    const result =
      await supabase
        ...
        .insert({
          ...
          min_value:
            numero(conceito.minimo),
          max_value:
            numero(conceito.maximo)
        });
  ...
  mostrarToast(
    "Dados antigos sincronizados com o banco."
  );
...
async function iniciar() {
  try {
    ...
    if (antigos) {
      ...
      mostrarToast(
        "Banco indisponível. Exibindo dados locais.",
        "warning"
      );
    } else {
      mostrarToast(
...
async function alterarPeriodo() {
  ...
  for (
    ...
    for (
      ...
      const valor =
        materia.trimestres[i]
          ?.valores?.[0] ?? "";

      await salvarNotaBanco(
        materia,
        i,
        valor,
        false
...
function calcularMateria(materia) {
  ...
  /*
   * IMPORTANTE:
   * Não existe divisão pela quantidade de avaliações.
   * O total é SOMENTE a soma das notas.
   */
  ...
  return {
    total,
    texto:
      `Total ${exibir(total)} · Falta ${exibir(falta)}`,
    classe: "recuperacao"
  };
}
...
async function adicionarConceito() {
  ...
  if (
    ...
    mostrarToast(
      "Preencha o conceito e uma faixa válida.",
      "warning"
    );
  ...
  if (error) {
    console.error(
      "Erro ao adicionar conceito:",
      error
    );

    mostrarToast(
      error.message ||
      "Não foi possível salvar o conceito.",
      "error"
    );
  ...
  dados.conceitos.push({
    id: data.id,
    nome: data.name,
    minimo: numero(data.min_value),
    maximo: numero(data.max_value)
  });
...
async function removerConceito(i) {
  ...
  if (
    !confirm(
      `Excluir o conceito "${conceito.nome}"?`
    )
  ) {
    return;
  }
...
async function adicionarMateria() {
  ...
  if (!input) {
    console.error(
      "Elemento #materia não encontrado."
    );

    mostrarToast(
      "Campo de matéria não encontrado.",
      "error"
    );
...
async function removerMateria(i) {
  ...
  if (
    !confirm(
      `Excluir a matéria "${materia.nome}" e todas as notas dela?`
    )
  ) {
    return;
  }
...
async function salvarNotaBanco(
  materia,
  periodo,
  valor,
  mostrar = true
...
async function alterarValorTrimestre(
  materiaIndex,
  periodoIndex,
  valor
) {
...
function renderizarConceitos() {
  if (!listaConceitos) return;

  listaConceitos.innerHTML = "";

  if (
    !dados.conceitos.length
  ) {
    listaConceitos.innerHTML = `
  ...
  dados.conceitos.forEach(
    (c, i) => {
      listaConceitos.insertAdjacentHTML(
        "beforeend",
        `
...
function renderizarMaterias() {
  ...
  if (
    ...
    listaMaterias.innerHTML = `
      <div class="empty-state large">
        ...
        <span>
          Adicione a primeira acima para começar
          a acompanhar seus pontos.
        </span>
      </div>
    `;
  ...
  dados.materias.forEach(
    (materia, index) => {
      ...
      listaMaterias.insertAdjacentHTML(
        "beforeend",
        `
 ...
 * para onclick="" no HTML.
 */
window.adicionarMateria =
  adicionarMateria;

window.removerMateria =
  removerMateria;

window.adicionarConceito =
  adicionarConceito;

window.removerConceito =
  removerConceito;

window.alterarValorTrimestre =
  alterarValorTrimestre;

window.alterarPeriodo =
  alterarPeriodo;
